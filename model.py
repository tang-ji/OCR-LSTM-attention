...
class Model(object):
    ...
    def test(self, data_path):
        ...
        for batch in s_gen.gen(1):
            ...
            log = open('log.txt', 'a')
            log.write('\nStep {:.0f} ({:.3f}s). Accuracy: {:6.2%}, loss: {:f}, perplexity: {:0<7.6}, probability: {:6.2%} {}'.format(
                         current_step,
                         curr_step_time,
    ...
    def train(self, data_path, num_epoch):
        ...
        log = open('log.txt', 'a')
        perplexity = math.exp(loss) if loss < 300 else float('inf')
        log.write("Global step %d. Time: %.3f, loss: %f, perplexity: %.2f."
                     % (self.sess.run(self.global_step), step_time, loss, perplexity))
        # Save checkpoint and reset timer and loss.
        log.write("Finishing the training and saving the model at step %d." % current_step)
        log.close()
        self.saver_all.save(self.sess, self.checkpoint_path, global_step=self.global_step)
    ...
    def visualize_attention(self, img_data, attentions, output, label, flag_incorrect):
        ...
        with open(os.path.join(output_dir, 'word.txt'), 'w') as fword:
            fword.write(output+'\n')
            fword.write(label)
            file_img_data = io.BytesIO(img_data)
            img = Image.open(file_img_data)
            w, h = img.size
            mh = 32
            mw = int(math.floor(1. * w / h * mh))
            img = img.resize(
                    (mw, h),
                    Image.ANTIALIAS)
